...
func main() {
	...
	// Check if sockstat file exists and is readable by trying to open it
...
// checkSockstatFile attempts to open the sockstat file to ensure it exists and is readable.
// This is generally a more reliable check than peeking at permission bits.
func checkSockstatFile() error {
	f, err := os.Open(sockstatPath)
	if err != nil {
		if os.IsNotExist(err) {
			return fmt.Errorf("'%s' not found. Ensure you are running on a Linux system", sockstatPath)
		}
		return fmt.Errorf("cannot open '%s': %v", sockstatPath, err)
	}
	f.Close()
	return nil
}

// logMessage writes log messages. To keep JSON output clean, logs are written to stderr when JSONOutput is true.
func logMessage(level, message string) {
	msgLevel, ok := logLevels[level]
	if !ok {
		// If an unknown level is passed, treat it as INFO
		msgLevel = logLevels["INFO"]
	}

	confLevel := logLevels[config.LogLevel]
	// Only output when the message level is at least as severe as configured level
	...
	// If JSON output is requested, write logs to stderr so stdout remains clean for JSON.
...
func parseLine(line string, stats *SocketStats) {
	// Use strings.Fields to split on any whitespace (handles multiple spaces/tabs)
...
func parseInt(s string) int {
	...
	if err != nil {
		// Use DEBUG level for parse failures
...
func outputJSON(stats *SocketStats) {
	...
	// JSON goes to stdout; logs (if any) are sent to stderr by logMessage.
